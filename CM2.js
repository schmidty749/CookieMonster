...
CM.Cache.RemakeFrenzyLuckyPP = function() {
	var GCSecs = 446;
	if(Game.Has('Lucky day')) GCSecs *= .5;
	if(Game.Has('Serendipity')) GCSecs *= .5;
	var GCupgrades = ['Heavenly luck','Golden goose egg','Arcane Aura'];
	for (var i in GCupgrades) {if (Game.Has(GCupgrades[i])) GCSecs*=0.95;}
	if (Game.season=='easter' && Game.Has('Starspawn')) GCSecs*=0.98;
	else if (Game.season=='halloween' && Game.Has('Starterror')) GCSecs*=0.98;
	else if (Game.season=='valentines' && Game.Has('Starlove')) GCSecs*=0.98;
	else if (Game.season=='fools' && Game.Has('Startrade')) GCSecs*=0.95;
	if (Game.hasGod)
	{
		var godLvl=Game.hasGod('industry');
		if (godLvl==1) GCSecs*=1.15;
		else if (godLvl==2) GCSecs*=1.1;
		else if (godLvl==3) GCSecs*=1.05;
		...
		if (Game.season!='')
		{
			...
			{
				if (godLvl==1) GCSecs*=0.97;
				else if (godLvl==2) GCSecs*=0.98;
				else if (godLvl==3) GCSecs*=0.99;
			}
			else
			{
				if (godLvl==1) GCSecs*=0.94;
				else if (godLvl==2) GCSecs*=0.96;
				else if (godLvl==3) GCSecs*=0.98;
			}
		}
	}
	var luckyChance = .47;
	var frenzyChance = .4;
	var dbuffs = 0;
	if (Game.hasAura('Reaper of Fields')) dbuffs++;
	if (Game.hasAura('Dragonflight')) dbuffs++;
	if (dbuffs == 1){
		luckyChance = .44;
		frenzyChance = .35;
	}
	...
	var GCIncome = 1;
	if(Game.cookies < CM.Cache.LuckyFrenzy)
	{
		GCIncome = (CM.Cache.LuckyRewardFrenzy - (Game.cookies*.15))*luckyChance*frenzyChance/GCSecs;
